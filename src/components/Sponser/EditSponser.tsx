...
import { useGetOneSponserQuery } from "@/redux/api/sponserApi/sponserAPi";
import { S3_URL } from "@/config";

const EditSponser = ({ sponserId }: Params) => {
  const { data, isError, isLoading, isSuccess } = useGetOneSponserQuery(
    sponserId,
    {
      skip: !sponserId,
    },
  ...
  useEffect(() => {
    if (isSuccess && data) {
      formik.setValues({
        ...
        largeTitle: data?.data?.largeTitle || "",
        shortContent: data?.data?.shortContent || "",
        logo: data?.logo || "",
        Existinglogo: data.logo ? `${S3_URL}${data.logo}` : "",
      });
    }
  }, [isSuccess, data]);
